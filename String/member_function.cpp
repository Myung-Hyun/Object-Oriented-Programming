...
int main( void )
{
	//find, substr, insert, append, replace, erase, compare 
	string s1, s2;

	//문자열 검색
	s1 = "01234560123456";
	int index = s1.find("23",0);//0번째 위치부터 검색하여 “23”의 substring의 시작위치를 찾아라.
	cout<< setw(10) << "find : " << s1 << " -> " << index << " : " << s1[index] << s1[index+1] <<endl;

	//문자열 추출
	s1 = "0123456";
	s2 = s1.substr(0,3);//0번째부터 3개의 문자를 잘라 할당하라.
	...
	s1.append(s2,0,s2.npos);//s2의 0~s2.npos 위치의 문자열을 s1의 맨뒤에 추가하라.
	cout << s1 << endl;
	...
	nCmp = s1.compare( 0, 4, s2, 0, 4 );//s1, s2의 0번째 부터 4개의 문자열을 비교하라.
	cout << setw(10) << "compare : " << ( (nCmp==0) ? "equal" : ( (nCmp<0) ? "less" : "greater" ) ) << endl;
}
